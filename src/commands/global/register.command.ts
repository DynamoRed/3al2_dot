...
const command: ICommand = {
    ...
    data: new SlashCommandBuilder()
            ...
            .addStringOption(opt => opt.setName('name').setDescription('Your name (Firstname LASTNAME)').setMinLength(7).setRequired(true))
    ...
    execute(interaction: CommandInteraction, app: Client) {
        const name: any = interaction.options.get('name')?.value;
        const email: any = interaction.options.get('email')?.value;
        ...
        UserUtil.discordIdToUuid(interaction.user.id).then(uuid => {
            ...
            new Promise<void>((resolve, reject) => {
                Database.pool.query("INSERT INTO members(uuid, discord_id, name, email, validation_code) VALUES (?,?,?,?,?)", [uuid, interaction.user.id, name, email, code], (err: MysqlError | null) => {
                    if(err){
